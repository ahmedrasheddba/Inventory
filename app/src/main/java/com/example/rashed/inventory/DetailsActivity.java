...
public class DetailsActivity extends AppCompatActivity {
    ...
    private void showUnsavedChangesDialog(
        ...
        builder.setMessage(R.string.unsaved_changes_messsage);
    ...
    private boolean addItemToDb() {
        ...
        if (!checkIfValueSet(priceEdit, "price_TxtView")) {
            isAllOk = false;
        }
        if (!checkIfValueSet(quantityEdit, "quantity_TxtView")) {
            isAllOk = false;
        }
        ...
        if (currentItemId == 0) {
            InventoryItem item = new InventoryItem(
                    ...
                    actualUri.toString());
            inventoryDBHelper.insertItem(item);
        } else {
            int quantity = Integer.parseInt(quantityEdit.getText().toString().trim());
            inventoryDBHelper.updateItem(currentItemId, quantity);
        }
        return true;
    }
    ...
    private void addValuesToEditItem(long itemId) {
        Cursor cursor = inventoryDBHelper.readItem(itemId);
        cursor.moveToFirst();
        nameEdit.setText(cursor.getString(cursor.getColumnIndex(InventoryContract.InventoryInput.PRODUCT_NAME)));
        priceEdit.setText(cursor.getString(cursor.getColumnIndex(InventoryContract.InventoryInput.PRODUCT_PRICE)));
        quantityEdit.setText(cursor.getString(cursor.getColumnIndex(InventoryContract.InventoryInput.PRODUCT_QUANTITY)));
        supplierNameEdit.setText(cursor.getString(cursor.getColumnIndex(InventoryContract.InventoryInput.SUPPLIER_NAME)));
        supplierPhoneEdit.setText(cursor.getString(cursor.getColumnIndex(InventoryContract.InventoryInput.SUPPLIER_PHONE)));
        supplierEmailEdit.setText(cursor.getString(cursor.getColumnIndex(InventoryContract.InventoryInput.SUPPLIER_EMAIL)));
        imageView.setImageURI(Uri.parse(cursor.getString(cursor.getColumnIndex(InventoryContract.InventoryInput.IMAGE))));
    ...
    private int deleteAllRowsFromTable() {
        ...
        return database.delete(InventoryContract.InventoryInput.TABLE_NAME, null, null);
    }
    ...
    private int deleteOneItemFromTable(long itemId) {
        ...
        int rowsDeleted = database.delete(
                InventoryContract.InventoryInput.TABLE_NAME, selection, selectionArgs);
        return rowsDeleted;
    }
